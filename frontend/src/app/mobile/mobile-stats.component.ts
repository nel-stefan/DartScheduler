    ...
    @if (!loading) {
      <div class="body">
        ...
        @for (row of rows; track row) {
          <div class="card" [class.dirty]="row.dirty">
        ...
        <button class="save-btn" [disabled]="saving || !hasDirty" (click)="saveAll()">
...
export class MobileStatsComponent implements OnInit {
  ...
  inc180(row: StatRow): void {
    row.oneEighties++;
    row.dirty = true;
  }
  dec180(row: StatRow): void {
    if (row.oneEighties > 0) {
      row.oneEighties--;
      row.dirty = true;
    }
  }
  markDirty(row: StatRow): void {
    row.dirty = true;
  }
  ...
  saveAll(): void {
    ...
    for (const row of dirty) {
      this.eveningStatSvc.upsert(this.selectedEveningId, row.playerId, row.oneEighties, row.highestFinish).subscribe({
        next: () => {
          row.dirty = false;
