...
func (r *ScheduleRepo) Save(ctx context.Context, s domain.Schedule) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO schedules(id,competition_name,season,created_at) VALUES(?,?,?,?)
         ON CONFLICT(id) DO UPDATE SET competition_name=excluded.competition_name,season=excluded.season`,
		s.ID.String(), s.CompetitionName, s.Season, s.CreatedAt)
	return err
}

func (r *ScheduleRepo) FindLatest(ctx context.Context) (domain.Schedule, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id,competition_name,season,created_at FROM schedules ORDER BY created_at DESC LIMIT 1`)
	return scanSchedule(row)
}

func (r *ScheduleRepo) FindByID(ctx context.Context, id domain.ScheduleID) (domain.Schedule, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id,competition_name,season,created_at FROM schedules WHERE id=?`, id.String())
	return scanSchedule(row)
}

func (r *ScheduleRepo) FindAll(ctx context.Context) ([]domain.Schedule, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id,competition_name,season,created_at FROM schedules ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func scanSchedule(s scanner) (domain.Schedule, error) {
	...
	var idStr string
	if err := s.Scan(&idStr, &sc.CompetitionName, &sc.Season, &sc.CreatedAt); err != nil {
